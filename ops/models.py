...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 ...
                 dropout=0.8, dropout_type='normal', img_feature_dim=256,
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            if self.temporal_module != 'none':
                ...
                make_temporal_shift(self.base_model, self.temporal_module, self.num_segments, self.args, 
                                    n_div=self.shift_div, place=self.shift_place, temporal_pool=self.temporal_pool)
                                    
            else:
                print('No temporal module')
            ...
            self.base_model.avgpool = nn.AdaptiveAvgPool2d(1)
            if self.temporal_module != 'none':
                from ops.temporal_shift import TemporalShift, get_temporal_module

                for m in self.base_model.modules():
                    if isinstance(m, InvertedResidual) and len(m.conv) == 8 and m.use_res_connect:
                        
                        if self.temporal_module == 'tsm':
                            if self.print_spec:
                            ...
                            print('adding {}...'.format(self.temporal_module))
                            t_m = get_temporal_module(self.temporal_module, m.conv[0].in_channels, self.num_segments,
                                n_div=8, i3d=False, args=self.args)
                            m.conv[0] = nn.Sequential(t_m, m.conv[0])
    ...
    def forward(self, input, no_reshape=False):
        if not no_reshape:
            ...
            if self.args.i3d: 
                h, w = input.size()[-2:]
                base_out = self.base_model(x.view(-1, 3, self.num_segments, h, w))
            else: 
